...
def main(argv):
    ...
    folder_name = args.checkpoint.split('/')[-1]
    folder_name = folder_name.split('.')[0]
    
    os.makedirs("image/output_%s"%(folder_name),exist_ok=True)
    p = 128
    path = args.input
    img_list = []
    ...
    if args.real:
        ...
        for img_name in img_list:
            ...
            with torch.no_grad():
                ...
                cv2.imwrite('image/output_%s/%s.png'%(folder_name,img_PATH),yi.astype(np.uint8))

        print('\n---result_bpp(real)---')
        print(f'average_Bit-rate: {(Bit_rate/len(img_list)):.3f} bpp')
        print('--- save image ---')
        print('compressed images are saved in "image/output_%s"\n'%(folder_name))
